...
app.use((state, emitter) => {
    ...
    emitter.on('setMessage', render(message => state.message = message))
...
const messageBar = (state, emit) => {
    function onInput(e) {
        emit('setMessage', e.target.value)
    }
    function onSubmit(e) {
        e.preventDefault()
        emit('sendMessage')
                ...
                <form class="navbar-form messageWidget" onsubmit=${onSubmit}>
                    <div class="input-group">
                        <input ${disableIf(!state.connected)} id="message" name="message"
                            ...
                            value=${state.message} oninput=${onInput} />
