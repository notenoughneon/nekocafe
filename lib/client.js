...
const cachedEmbed = cache(embedRow, (curr, prev) => {
    console.log('isSame: ');
...
const messageItem = (state, message, prev, installOnload) => {
    let embeds = state.boss ? [] : message.embeds;
    embeds = zip(embeds, prev ? prev.embeds : []);
    if (installOnload)
        return html`
            <li onload=${scrollDown}>
                ...
                ${embeds.map(elt => cachedEmbed(elt[0], elt[1]))}
            </li>
        `;
    else
        return html`
...
const messageList  = (state, prev) => {
    const messages = zip(state.messages, prev.messages);
    return html`
        <ul class="messageList">
            ${messages.map((elt, i) => messageItem(state, elt[0], elt[1], i+1 === messages.length))}
        </ul>
    `;
};
