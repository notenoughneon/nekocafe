...
app.use((state, emitter) => {
    ...
    emitter.on('login', () => {
        ...
        socket = io()
        socket.on('connect', () => {
            socket.emit('nick', state.nick)
            socket.emit('replay', lastMessage())
...
const embedRow = (message, embed) => {
    var inner = html`<div></div>`
    switch (embed.type) {
        case 'html':
            inner.innerHTML = embed.html
            break
        case 'tweet':
            inner = tweet(message.time, embed.id)
            break
        default:
...
const messageItem = (state, message) => {
    ...
    return html`
        <li>
            ...
            ${embeds.map(embed => embedRow(message, embed))}
        </li>
    `
}
...
const messageList  = (state) => {
    ...
    return html`
        <ul class="messageList">
            ${messages.map((message, i) => messageItem(state, message))}
        </ul>
    `
}
