...
app.model({
    ...
    reducers: {
        ...
        setDragging: (state, dragging) => ({dragging: dragging})
    },
    effects: {
        sendMessage: (state, data, send, done) => {
            socket.emit('message', data);
        },
        sendFile: (state, data, send, done) => {
            socket.emit('file', {name: data.name, type: data.type, data: data});
        },
...
const optionsWidget = (state, send) => {
    function handleFiles(e) {
        send('sendFile', e.target.files[0]);
    }
    return html`
...
const mainView = (state, prev, send) => {
    ...
    function dragleave(e) {
        e.stopPropagation();
        e.preventDefault();
        send('setDragging', false);
    }
    function drop(e) {
        e.stopPropagation();
        e.preventDefault();
        if (e.dataTransfer.files[0]) {
            send('sendFile', e.dataTransfer.files[0]);
        }
        send('setDragging', false);
    }
    return html`
        <div class="main ${state.dark ? 'dark' : ''}"
