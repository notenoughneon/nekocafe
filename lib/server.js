...
io.on('connection', socket => {
    ...
    var me
    socket.on('nick', nick => {
        console.log(socket.conn.remoteAddress + ' nick ' + nick)
        if (me === undefined) {
    ...
    socket.on('replay', since => {
        console.log(socket.conn.remoteAddress + ' replay ' + since)
        if (me !== undefined) {
            messages.filter(m => m.value.time > since).forEach(m => unicast(socket, m))
        }
    })
